...
function doRectanglesOverlap(rect1, rect2) {
  const lAng1 = rect1.left;
  const rAng2 = rect2.left + rect2.width;
  const rAng1 = rect1.left + rect1.width;
  const bAng1 = rect1.top + rect1.height;
  const bAng2 = rect2.top + rect2.height;
  if (!(lAng1 > rAng2) && !(rect2.left > rAng1) && !(bAng1 < rect2.top) && !(bAng2 < rect1.top)) {
    return true;
  }
  return false;
}
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < revCcnStr.length; i += 1) {
    if ((i + 1) % 2 !== 0) {
      transformedStr += revCcnStr[i];
    }
    if ((i + 1) % 2 === 0) {
      let mult = +revCcnStr[i] * 2;
      if (mult > 9) {
        mult -= 9;
      }
      transformedStr += mult;
  ...
  let sum = 0;
  for (let i = 0; i < transformedStr.length; i += 1) {
    sum += +transformedStr[i];
  }
  if (sum % 10 === 0) {
    return true;
  }
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (bracketOpen.includes(current)) {
      stack.push(current);
    } else if (bracketClose.includes(current)) {
      const exp1 = (current === ']' && top === '[');
      const exp2 = (current === ')' && top === '(');

      if (exp1 || exp2 || (current === '}' && top === '{') || (current === '>' && top === '<')) {
        stack.pop();
      } else {
        return false;
      }
...
function getCommonDirectoryPath(pathes) {
  const array = pathes.map((item) => item.split('/'));
...
function getMatrixProduct(m1, m2) {
  ...
  const mult = [];
  for (let i = 0; i < rows1; i += 1) mult[i] = [];
